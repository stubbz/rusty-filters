    ...
    fn plot_freq_response(){
...

        {
            ...
            fn add_line(axes: &mut Axes2D, xs: &[f32], cutoff: f32, quality_factor: f32, sample_rate: SampleRate, transform_2: &BilinearTransform, transform_3: &BilinearTransform, transform_4: &BilinearTransform, transform_5: &BilinearTransform){
                ...
                let hpf4 = butterworth_hpf4(NormalizedFrequency(cutoff), QualityFactor(quality_factor), &transform_4).frequency_response(sample_rate);
				...
				let mooglpf4 = moog_lpf4(NormalizedFrequency(cutoff), QualityFactor(quality_factor), &transform_4).frequency_response(sample_rate);
                ...
                axes
					...
					//.lines(xs.iter(), mooglpf4.iter().map(|y| y.from_voltage_ratio_to_db()), &[Color("black")])
            ...
            let qualities = vec![0.0, 0.3, 0.6, 0.9, 0.99];
			let qualities = vec![0.5];
...
fn peak_eq_prototype(db: f32, q: f32) -> DigitalFilterPrototype {
    let g = db.from_db_to_voltage_ratio();
    println!("{}", g);
    let b = 1.0 / (q);
...
#[inline]
fn moog_lpf4(normalized_frequency: NormalizedFrequency, quality_factor: QualityFactor, transform: &BilinearTransform) -> DigitalFilter{//todo return result
    use std::f32::consts;

    let big_omega = normalized_frequency.prewarp();
	
    let feed_back_coefficients = {
        let QualityFactor(q) = quality_factor;
		let gain = 4.0;
		let k = gain * q;
